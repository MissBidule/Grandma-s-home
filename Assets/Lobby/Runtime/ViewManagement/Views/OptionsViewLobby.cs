...
namespace PurrLobby
{
    public class OptionsViewLobby : MonoBehaviour
    {
        [Header("Panels")]
        ...
        {
            if (_programmatic) if (resetButton) resetButton.onClick.RemoveAllListeners();
        }
        ...
        {
            if (videoPanel && videoPanel.gameObject.activeSelf)         videoPanel.ResetToDefaults();
            if (audioPanel && audioPanel.gameObject.activeSelf)         audioPanel.ResetToDefaults();
            if (accessibilityPanel && accessibilityPanel.gameObject.activeSelf) accessibilityPanel.ResetToDefaults();
            if (controlsPanel && controlsPanel.gameObject.activeSelf)      controlsPanel.ResetToDefaults();
        }
